...
int main() {
  ...
  heap_t hp = crear_heap(MAX_HEAP, MAX_VALOR);
  hash_t hash = crear_hash(MAX_HASH);
  ...
  while (!salir) {
    ...
    if (!strcmp(nom_comando, "Fin")) {
      ...
      // operaciones de hash
    } else if (!strcmp(nom_comando, "asociar_en_hash")) {
      if (esta_lleno_hash(hash))
        printf("El hash está lleno.\n");
      else {
        int clave;
        scanf("%d", &clave);
        if (existe_asociacion(clave, hash))
          printf("Ya existe asociación.\n");
        else {
          char *valor = new char[MAX_PALABRA];
          scanf("%s", valor);
          asociar_en_hash(clave, valor, hash);
          printf("Establecida la asociación.\n");
        }
      }

    } else if (!strcmp(nom_comando, "actualizar_hash")) {
      int clave;
      scanf("%d", &clave);
      if (!existe_asociacion(clave, hash))
        printf("No existe asociación.\n");
      else {
        char *valor = new char[MAX_PALABRA];
        scanf("%s", valor);
        actualizar_hash(clave, valor, hash);
        printf("Actualizada la asociación.\n");
      }

    } else if (!strcmp(nom_comando, "eliminar_de_hash")) {
      int clave;
      scanf("%d", &clave);
      if (!existe_asociacion(clave, hash))
        printf("No existe asociación.\n");
      else {
        eliminar_de_hash(clave, hash);
      ...
      int clave;
      scanf("%d", &clave);
      printf("%s.\n", existe_asociacion(clave, hash) ? "Existe asociacion"
                                                     : "No existe asociacion");

    } else if (!strcmp(nom_comando, "valor_en_hash")) {
      int clave;
      scanf("%d", &clave);
      if (!existe_asociacion(clave, hash))
        printf("No existe asociación.\n");
      else
        printf("%s\n", valor_en_hash(clave, hash));

    } else if (!strcmp(nom_comando, "esta_lleno_hash")) {
      printf("%s.\n", esta_lleno_hash(hash) ? "LLeno" : "No lleno");

      // operaciones de heap

    } else if (!strcmp(nom_comando, "insertar_en_heap")) {
      if (es_lleno_heap(hp))
        printf("El heap está lleno.\n");
      else {
        info_t info = leer_info(MAX_PALABRA);
        nat v = numero_info(info);
        if ((v > max_valor(hp)) || (hay_valor(v, hp))) {
          if (v > max_valor(hp))
            printf("Valor fuera de rango.\n");
          else
            printf("Ya existe un elemento con ese valor.\n");
          liberar_info(info);
        } else {
          insertar_en_heap(info, hp);
          printf("Se insertó el elemento.\n");
        }
      }

    } else if (!strcmp(nom_comando, "reducir")) {
      nat v;
      scanf("%d", &v);
      if (!hay_valor(v, hp))
        printf("No existe ese valor.\n");
      else if (hay_valor(v / 2, hp))
        printf("Ya existe un elemento con la mitad de ese valor.\n");
      else {
        reducir(v, hp);
        printf("Se redujo el valor.\n");
      }

    } else if (!strcmp(nom_comando, "eliminar_menor")) {
      if (es_vacio_heap(hp))
    ...
    } else if (!strcmp(nom_comando, "menor")) {
      if (es_vacio_heap(hp))
        printf("El heap está vacío.\n");
      else {
        char *txt_info = info_a_texto(menor(hp));
        printf("%s\n", txt_info);
        delete[] txt_info;
      }

    } else if (!strcmp(nom_comando, "es_vacio_heap")) {
      printf("%s.\n", es_vacio_heap(hp) ? "Vacio" : "No vacío");

    } else if (!strcmp(nom_comando, "es_lleno_heap")) {
      printf("%s.\n", es_lleno_heap(hp) ? "LLeno" : "No lleno");

    } else if (!strcmp(nom_comando, "hay_valor")) {
      nat v;
      scanf("%d", &v);
      printf("%s.\n", hay_valor(v, hp) ? "Hay valor" : "No hay valor");

    } else if (!strcmp(nom_comando, "max_valor")) {
      printf("Máximo valor posible: %d.\n", max_valor(hp));
      ...
      conj = crear_conjunto();
      liberar_heap(hp);
      hp = crear_heap(MAX_HEAP, MAX_VALOR);
      liberar_hash(hash);
      hash = crear_hash(MAX_HASH);
    ...
    } else if (!strcmp(nom_comando, "tiempo_hash")) {
      nat tamanio;
      scanf("%d", &tamanio);
      const nat MODULO = 1000000;
      srand(1);
      hash_t hsh_tmp = crear_hash(tamanio);
      printf("prueba actualizar. \n");
      nat rnd = rand() % MODULO;
      asociar_en_hash(rnd, int_a_texto(rnd), hsh_tmp);
      for (nat j = 1; j < tamanio; j++) {
        actualizar_hash(rnd, int_a_texto(rnd), hsh_tmp);
      }
      printf("prueba existe_asociacion, valor_en_hash. \n");
      for (nat j = 0; j < MODULO; j++) {
        if (existe_asociacion(rnd, hsh_tmp))
          valor_en_hash(rnd, hsh_tmp);
      }
      liberar_hash(hsh_tmp);

    } else if (!strcmp(nom_comando, "tiempo_heap")) {
      nat max_prioridades;
      scanf("%d", &max_prioridades);
      const nat tamanio = max_prioridades;
      char *txt_nulo;
      info_t info;
      printf("\n prueba 1. \n");
      heap_t hp_tmp = crear_heap(tamanio, max_prioridades);
      for (nat i = tamanio; i >= 1; i--) {
        txt_nulo = new char[1];
        txt_nulo[0] = '\0';
        info_t info = crear_info(i, txt_nulo);
        insertar_en_heap(info, hp_tmp);
      }
      for (nat i = 1; i <= tamanio; i++)
        eliminar_menor(hp_tmp);

      liberar_heap(hp_tmp);

      printf(" prueba 2. \n");
      hp_tmp = crear_heap(tamanio, max_prioridades);
      for (nat i = 1; i <= tamanio; i++) {
        txt_nulo = new char[1];
        txt_nulo[0] = '\0';
        info_t info = crear_info(i, txt_nulo);
      ...
      txt_nulo = new char[1];
      txt_nulo[0] = '\0';
      info = crear_info(max_prioridades, txt_nulo);
      insertar_en_heap(info, hp_tmp);
      for (nat i = 1; i <= tamanio; i++) {
        txt_nulo = new char[1];
        txt_nulo[0] = '\0';
        info_t info = crear_info(1, txt_nulo);
        insertar_en_heap(info, hp_tmp);
        eliminar_menor(hp_tmp);
      }
      liberar_heap(hp_tmp);
      ...
      for (nat i = 1; i <= tamanio; i++) {
        hay_valor(0, hp_tmp);
        hay_valor(max_prioridades / 2, hp_tmp);
        hay_valor(max_prioridades, hp_tmp);
      }
      liberar_heap(hp_tmp);
